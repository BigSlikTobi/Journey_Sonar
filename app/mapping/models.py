"""SQLAlchemy models for the Mapping Engine — schema: mapping."""
...
class MappingRule(Base):
    ...
    target_node_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), index=True, nullable=False)
    priority: Mapped[int] = mapped_column(Integer, default=0)
    conditions: Mapped[dict] = mapped_column(JSONB, nullable=False)
    signal_type: Mapped[SignalType] = mapped_column(
        Enum(SignalType, schema="mapping"), nullable=False
    )
...
class MappedSignal(Base):
    ...
    normalized_event_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False)
    mapping_rule_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False)
...
class AutoMappingSuggestion(Base):
    ...
    workspace_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), index=True, nullable=False)
    normalized_event_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False)
    suggested_node_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False)
    confidence: Mapped[float] = mapped_column(Float, nullable=False)
    reasoning: Mapped[str] = mapped_column(String(1000), default="")
    status: Mapped[SuggestionStatus] = mapped_column(
        Enum(SuggestionStatus, schema="mapping"), default=SuggestionStatus.PENDING
    )
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
